...
class BEM4PNP1D:
    ...
    def printResult4Paper(self, cs, phi, fileName):
        ...
        # ax1.set_ylabel(r' $\phi(x)$', fontsize=fontsize)
        ax1.set_title('(a) potential' + r' $\phi(x)$', fontsize=fontsize, pad=20)
        ax3.set_title('(b) anion concentration'+r' $c_1(x)$', fontsize=fontsize, pad=20)
        # ax2.set_ylabel(r' $\phi^\prime$', fontsize=30)
        # ax3.set_ylabel(r' $c_1(x)$', fontsize=fontsize)
...
if __name__ == "__main__":
    ...
    N = 100
    fileName = 'Electroneutral_Case_21.png'
    t1 = perf_counter()
    plt.figure(num=2)
    ...
    for (eta, omega, label, color, marker) in zip(etas, omegas, labels, colors, markers):
        ...
        for N in [100]:
            ...
            ax1.plot(xs, phi, label=label, linewidth=4, color=color, marker=marker, markevery=[10,90],
                     markersize=25, markerfacecolor='None',  markeredgewidth=5)
            ax3.tick_params(which='both', length=10, width=4, direction="in")
            # ax2.plot(xs, Dphi, label=label, linewidth=4)
            ax3.plot(xs, cs[0], label=label, linewidth=4, color=color, marker=marker, markevery=[10,90],
                     markersize=25, markerfacecolor='None', markeredgewidth=5)
            # ax4.plot(xs, cs[1], label=label, linewidth=4)
            #
    ...
    ax1.set_title('(a) potential' + r' $\phi$, ' + r'$\epsilon=1/4$', fontsize=fontsize, pad=20)
    ax3.set_title('(b) anion concentration' + r' $c_1$, ' + r'$\epsilon=1/4$', fontsize=fontsize, pad=20)
    # ax2.set_ylabel(r' $\phi^\prime$', fontsize=30)
    # ax3.set_ylabel(r' $c_1(x)$', fontsize=40)
    # ax4.set_ylabel('Concentration ' + r' $c+$', fontsize=30)
    ...
    # Test Poisson Nernst Planck equation
    # N = 100
    fileName = 'Electroneutral_Case_22.png'
    t1 = perf_counter()
    plt.figure(num=10)
    fig, (ax1, ax3) = plt.subplots(1, 2, figsize=(27, 12))
    cs = []
    ...
    css = []
    # # --------------------------------------------------------------
    omegas = [0.09, 0.09, 0.09, 0.02, 0.02]
    etas = [0, epsilon ** 2, epsilon, np.sqrt(epsilon), 1]
    # Test Poisson Nernst Planck equation
    labels = [r'$\eta=0$', r'$\eta=\epsilon^2$', r'$\eta=\epsilon$', r'$\eta=\epsilon^{1/2}$', r'$\eta=1$']
    # labels = [r'$\epsilon=1/64, \eta=0$', r'$\epsilon=1/64,\eta=\epsilon^2$', r'$\epsilon=1/64,\eta=\epsilon$',
    #           r'$\epsilon=1/64, \eta=\epsilon^{1/2}$', r'$\epsilon=1/64, \eta=1$']
    for (eta, omega, label, color, marker) in zip(etas, omegas, labels, colors, markers):
        # for (eta, omega) in zip([0, epsilon**2, epsilon, np.sqrt(epsilon), 1], [0.001,0.00001, 0.00001,0.00001,0.00001]):
        # chi2 = 1/epsilon
        # eta = epsilon
        for N in [300]:
            ...
            # ax1.plot(xs, phi, label=label, linewidth=4, color=color, marker=marker,
            #          markevery=[5, 95], markersize=25, markerfacecolor='None', markeredgewidth=5)
            # a = np.arange(len(xs))
            a =xs
            ax1.plot(a, phi, label=label, linewidth=4, color=color, marker=marker,
                     markevery=[15, 285], markersize=25, markerfacecolor='None', markeredgewidth=5)  # we plot y as a function of a, which parametrizes x

            ax3.tick_params(which='both', length=10, width=4, direction="in")
            # ax2.plot(xs, Dphi, label=label, linewidth=4)
            ax3.plot(a, cs[0], label=label, linewidth=4, color=color,  marker=marker,
                     markevery=[15, 285], markersize=25, markerfacecolor='None', markeredgewidth=5)
            # ax4.plot(xs, cs[1], label=label, linewidth=4)
    fontsize = 40
    for axis in ['top', 'bottom', 'left', 'right']:
        ax1.spines[axis].set_linewidth(4)
    ...
    ax1.xaxis.set_tick_params(labelsize=fontsize)
    ax1.yaxis.set_tick_params(labelsize=fontsize)
    # ax1.xaxis.set_ticks([a[0],a[75], a[150], a[225], a[-1]])  # set the ticks to be a
    # ax1.xaxis.set_ticklabels(["{:.1f}".format(xs[0]),"{:.1f}".format(xs[75]),
    #                           "{:.1f}".format(xs[170]),"{:.1f}".format(xs[225]), "{:.1f}".format(xs[-1])])  # change the ticks' names to x
    # ax2.xaxis.set_tick_params(labelsize=24)
    # ax2.yaxis.set_tick_params(labelsize=24)
    ax3.xaxis.set_tick_params(labelsize=fontsize)
    ax3.yaxis.set_tick_params(labelsize=fontsize)
    # ax3.xaxis.set_ticks([a[0], a[75], a[150], a[225], a[-1]])  # set the ticks to be a
    # ax3.xaxis.set_ticklabels(["{:.1f}".format(xs[0]), "{:.1f}".format(xs[75]),
    #                           "{:.1f}".format(xs[170]), "{:.1f}".format(xs[225]),
    #                           "{:.1f}".format(xs[-1])])  # change the ticks' names to x
    # ax4.xaxis.set_tick_params(labelsize=24)
    # ax4.yaxis.set_tick_params(labelsize=24)
    ax1.set_xlabel(r'$x$', fontsize=fontsize)
    # ax2.set_xlabel('x', fontsize=24)
    ...
    # ax4.set_xlabel('x', fontsize=24)
    # ax1.set_ylabel(r' $\phi(x)$', fontsize=40)
    ax1.set_title('(c) potential' + r' $\phi$, ' + r'$\epsilon=1/64$', fontsize=fontsize, pad=20)
    ax3.set_title('(d) anion concentration' + r' $c_1$, ' + r'$\epsilon=1/64$', fontsize=fontsize, pad=20)
    # ax2.set_ylabel(r' $\phi^\prime$', fontsize=30)
    # ax3.set_ylabel(r' $c_1(x)$', fontsize=40)
    # ax4.set_ylabel('Concentration ' + r' $c+$', fontsize=30)
    ax1.legend(fontsize=fontsize-5, loc=2)
    # ax2.legend(fontsize=24)
    ax3.legend(fontsize=fontsize-5)
    # ax4.legend(fontsize=24)
    plt.subplots_adjust(left=0.1, bottom=0.1, right=0.95, top=0.9, wspace=0.4, hspace=0.5)
